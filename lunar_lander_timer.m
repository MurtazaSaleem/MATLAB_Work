...
function main()
    ...
    % Start a timer with callback function 'timer_callback'
    t = timer;
    ...
    % Loop until 'run' is set false, the main part of the simulation happens in the timer callback
    while run
        %pause(0.9);
    ...
    stop(t);
    run = false;   % Ensure callback stops doing anything in case we exited the loop above by 'break'
    delete(t);
    return
    
    %** The following functions are nested so they can access variables in the main function

    % Timer callback for the simulation main loop, executes every TIME_STEP seconds
    function timer_callback(~, ~)    
        if ~run      % This is needed because the timer might trigger a few times after 'run' is set false 
            return;
        end

        % Determine commanded thrust
        thrust = 0;
        if spacePressed
        ...
        addpoints(line, 0, 0, height);

        % Display results and quit when lander touches down
        if landed
